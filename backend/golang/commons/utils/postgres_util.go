...
func NewPostgresConnection() PostgresUtil {
	println(time.Now().String(), "postgres: connecting to", os.Getenv("ECOMMERCEV2_POSTGRES_HOST"))
	ctx := context.Background()
	connectionString := "postgres://" + os.Getenv("ECOMMERCEV2_POSTGRES_USERNAME") + ":" + os.Getenv("ECOMMERCEV2_POSTGRES_PASSWORD") + "@" + os.Getenv("ECOMMERCEV2_POSTGRES_HOST") + "/" + os.Getenv("ECOMMERCEV2_POSTGRES_DATABASE")
	config, err := pgxpool.ParseConfig(connectionString)
	if err != nil {
		log.Fatalln("error when parse config: " + err.Error())
	}

	maxConnection, err := strconv.Atoi(os.Getenv("ECOMMERCEV2_POSTGRES_MAX_CONNECTION"))
	if err != nil {
		log.Fatalln("error when converting max connection: " + err.Error())
	}
	config.MaxConns = int32(maxConnection)

	maxConnectionIdletime, err := strconv.Atoi(os.Getenv("ECOMMERCEV2_POSTGRES_MAX_IDLETIME"))
	if err != nil {
	...
	maxConnectionLifetime, err := strconv.Atoi(os.Getenv("ECOMMERCEV2_POSTGRES_MAX_LIFETIME"))
	if err != nil {
	...
	if err != nil {
		log.Fatalln("error when connecting to " + os.Getenv("ECOMMERCEV2_POSTGRES_HOST") + ", error:" + err.Error())
	}
	...
	println(time.Now().String(), "postgres: connected to", os.Getenv("ECOMMERCEV2_POSTGRES_HOST"))
